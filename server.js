...
app.post('/orders', (req, res) => {
    ...
    // Validate input data
    if (!CropID || !TotalPrice || !buyerid) {
        res.status(400).json({ message: 'Invalid input. All fields are required.' });
        return;
    }

    // SQL query to insert the order into the database
    const sqlInsert = 'INSERT INTO orders (CropID, TotalPrice, vendorid) VALUES (?, ?, ?)'; // Updated comment
    db.query(sqlInsert, [CropID, TotalPrice, buyerid], (err, result) => {
        ...
        // Respond with the inserted order ID
        res.status(200).json({
...
app.post('/admin/login', (req, res) => {
    ...
    if (username === 'lokesh' && password === '1234') {
